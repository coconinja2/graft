...
exports.graftTools = graftTools;
const client_1 = require("../../sdk/client");
function graftTools(options) {
    const client = new client_1.GraftClient(options);
    return [
        {
            name: 'graft_claim',
            ...
            execute: async (args) => {
                ...
                if (result.granted)
                    return JSON.stringify({ granted: true });
        ...
        },
        {
            ...
            execute: async (args) => {
                const released = await client.release(args.resource_id);
            ...
            execute: async (args) => {
                const signal = await client.publish({
                    ...
                    affectedResources: args.affected_resources
                        ? args.affected_resources.split(',').map(s => s.trim())
                        : undefined,
        ...
        },
        {
            ...
            execute: async (args) => {
                const released = await client.releasePool(args.pool_name, args.resource);
                return JSON.stringify({ released });
            },
        },
        {
            ...
            execute: async (args) => {
                const result = await client.waveComplete(args.wave_name);
