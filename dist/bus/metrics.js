...
function histogramSummary(values, upperBounds) {
    if (values.length === 0) {
        return {
            count: 0, sum: 0, min: 0, max: 0, p50: 0, p95: 0, p99: 0,
            buckets: [...upperBounds.map(le => ({ le: le, count: 0 })), { le: '+Inf', count: 0 }],
        };
    }
    const sorted = [...values].sort((a, b) => a - b);
    const sum = sorted.reduce((acc, v) => acc + v, 0);
    const pct = (p) => sorted[Math.max(0, Math.ceil(sorted.length * p / 100) - 1)];
    const buckets = upperBounds.map(le => ({
        le,
...
class MetricsCollector {
    constructor(audit, registry, signals, pool) {
        this.audit = audit;
    ...
    computeCounters() {
        ...
        for (const e of this.entries()) {
            switch (e.type) {
                case 'claim_granted':
                    counts.claims_granted_total++;
                    break;
                case 'claim_denied':
                    counts.claims_denied_total++;
                    break;
                case 'claim_expired':
                    counts.claims_expired_total++;
                    break;
                case 'claim_released':
                    counts.claims_released_total++;
                    break;
                case 'signal_published':
                    counts.signals_published_total++;
                    break;
                case 'signal_delivered':
                    counts.signals_delivered_total++;
                    break;
                case 'pool_acquired':
                    counts.pool_acquired_total++;
                    break;
                case 'pool_released':
                    counts.pool_released_total++;
                    break;
                case 'deadlock_detected':
                    counts.deadlocks_detected_total++;
                    break;
                case 'deadlock_resolved':
                    counts.deadlocks_resolved_total++;
                    break;
            }
        }
        return counts;
    ...
    computeHistograms() {
        const grantTimes = new Map(); // `${agentId}:${resourceId}` → ts
        const publishTimes = new Map(); // signalId → ts
        const holdDurations = [];
    ...
    computeContention(limit = 20) {
        const stats = new Map();
        for (const e of this.entries()) {
            if (e.type !== 'claim_denied' || !e.resourceId)
                continue;
            ...
            const holder = e.detail.holder;
            if (holder)
                s.blockers.set(holder, (s.blockers.get(holder) ?? 0) + 1);
        }
        const topN = (m) => Array.from(m.entries()).sort((a, b) => b[1] - a[1]).slice(0, 3).map(([agentId, count]) => ({ agentId, count }));
    ...
    computeEfficiency() {
        const stats = new Map();
        for (const e of this.entries()) {
            if (e.type !== 'claim_granted' && e.type !== 'claim_denied')
                continue;
            if (!stats.has(e.agentId))
                stats.set(e.agentId, { granted: 0, denied: 0, blockedMs: [] });
        ...
        return Array.from(stats.entries()).map(([agentId, s]) => {
            ...
            const avgBlockedMs = s.blockedMs.length > 0
                ...
                : 0;
            const efficiencyScore = Math.round((1 - blockRate / 100) * Math.max(0, 1 - avgBlockedMs / 60000) * 100);
    ...
    computeAgentRoster() {
        ...
        for (const e of this.entries()) {
            ...
            if (e.type === 'claim_granted')
                s.granted++;
            else if (e.type === 'claim_denied')
                s.denied++;
            else if (e.type === 'signal_published')
                s.published++;
            else if (e.type === 'signal_delivered')
                s.received++;
        }
        const claimsByAgent = new Map();
        for (const c of this.registry.list()) {
            if (!claimsByAgent.has(c.agentId))
                claimsByAgent.set(c.agentId, []);
