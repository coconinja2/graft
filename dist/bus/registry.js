...
class ClaimRegistry {
    ...
    claim(req) {
        ...
        for (const c of existing) {
            if (c.claimType !== 'write')
                continue;
            if (c.expiresAt <= now)
                continue;
            if (c.agentId === req.agentId)
                continue;
            if (!rangesOverlap(c.lineRange, req.lineRange))
                continue;
        ...
        if (!this.claims.has(req.resourceId))
            this.claims.set(req.resourceId, []);
    ...
    release(resourceId, agentId, lineRange) {
    ...
    notifyWaiters(resourceId) {
        ...
        if (this.get(resourceId).length > 0)
            return;
