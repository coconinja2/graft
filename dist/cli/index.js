...
hook
    ...
    const toolName = opts.tool ?? stdinData.tool_name ?? '';
    const agentId = opts.agent ?? process.env.GRAFT_AGENT_ID ?? stdinData.session_id ?? 'unknown';
    let toolInput = stdinData.tool_input ?? {};
    if (opts.input)
        try {
...
hook
    ...
    let toolOutput = stdinData.tool_output;
    if (opts.output)
        try {
            toolOutput = JSON.parse(opts.output);
        }
        catch { /* ignore */ }
...
program
    ...
    try {
        if (opts.json) {
            const data = await api('GET', '/metrics?format=json');
...
program
    ...
    .action(async (opts) => {
    try {
        let roster = await api('GET', '/agents');
        if (opts.active)
            roster = roster.filter(a => a.active);
        if (roster.length === 0) {
        ...
        for (const a of roster) {
            ...
            if (a.currentClaims.length > 0)
                console.log(`  holding: ${a.currentClaims.join(', ')}`);
            if (a.pendingSignals > 0)
                console.log(`  pending signals: ${a.pendingSignals}`);
        }
    }
    catch (e) {
        console.error('Bus unreachable:', e.message);
...
stats
    ...
    try {
        ...
        const list = await api('GET', `/stats/contention${params}`);
        if (list.length === 0) {
            console.log('No contention recorded.');
            return;
        }
